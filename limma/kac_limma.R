...
p1_input <- read.csv("p1_limma_export_kac.csv", row.names=1)
p2_input <- read.csv("p2_limma_export_kac.csv", row.names=1)

# Convert to log2
p1_log2 <- log2(p1_input)
p2_log2 <- log2(p2_input)
...
# Export the data
write.fit(p1_fit_cont_eb, p1_fit_cont_eb_decide, "p1_eb_fit_kac.tsv")
write.fit(p2_fit_cont_eb, p2_fit_cont_eb_decide, "p2_eb_fit_kac.tsv")